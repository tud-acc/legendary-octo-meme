...
function setBomb(lobby, team, coord){
    ...
    if(team == 'A'){
        if(lobby.teamA.gadgets.bombe < 0){ // prüfe ob Team bombe setzen darf
            return lobby;
        }
        lobby.teamA.gadgeds.bombe --;
        owner=1;
    }else{
        if(lobby.teamB.gadgets.bombe < 0){ // prüfe ob Team bombe setzen darf
            return lobby;
        }
...
function distance(p1, p2){
    ...
    let d = r * Math.acos( Math.sin(lat2) * Math.sin(lat1)
